...
def _train(cfg, _run, ex, tensorboard):
    ...
    for task_i in range(inc_dataset.n_tasks):
        task_info, train_loader, val_loader, test_loader, new_data_loader = inc_dataset.new_task()
        ...
        if task_i == 0 and cfg["start_class"] > 0:
            ...
            do_pretrain(cfg, ex, model, device, train_loader, test_loader, tensorboard)
            ...
            model.before_task(task_i, inc_dataset, new_data_loader)
...
def do_pretrain(cfg, ex, model, device, train_loader, test_loader, tensorboard=None):
    if not os.path.exists(osp.join(ex.base_dir, 'pretrain/')):
        os.makedirs(osp.join(ex.base_dir, 'pretrain/'))
    model_path = osp.join(
        ex.base_dir,
        "pretrain/{}_{}_cosine_{}_multi_{}_aux{}_nplus1_{}_{}_trial_{}_{}_seed_{}_start_{}_epoch_{}_calibrated_{}_calibhead_{}_calibbone_{}_multihead_{}_pruning_{}_lambda_{}.pth".format(
            cfg["model"],
            cfg["convnet"],
            ...
            cfg["start_class"],
            cfg["pretrain"]["epochs"],
            cfg["novelty_detection"]["enable"],
            cfg["novelty_detection"]["calibration_head"],
            cfg["novelty_detection"]["calibration_backbone"],
            cfg["novelty_detection"]["multihead"],
            cfg["pruning"]["enable"],
            cfg["pruning"]["lambda"],
        ),
    )
    if osp.exists(model_path):
        ...
        if cfg['continue_pretrain']:
            pretrain(cfg, ex, model, device, train_loader, test_loader, model_path, tensorboard)
    else:
        pretrain(cfg, ex, model, device, train_loader, test_loader, model_path, tensorboard)
...
def test(_run, _rnd, _seed):
    ...
    for taski in range(inc_dataset.n_tasks):
        task_info, train_loader, _, test_loader, _ = inc_dataset.new_task()
        model.set_task_info(
            task=task_info["task"],
            total_n_classes=task_info["max_class"],
            increment=task_info["increment"],
        ...
        model.before_task(taski, inc_dataset, train_loader)
