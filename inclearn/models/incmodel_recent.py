...
class IncModel(IncrementalLearner):
        ...
        if self._der:
            ...
            for i in range(self._num_heads):
                if i != self._head_to_train:
                    ...
                    self._parallel_network.module.convnets[i].train()
                    if self._parallel_network.module.use_calibration_backbone:
                        self._parallel_network.module.convnets_calib[i].train()
        else:
            self._parallel_network.train()
    ...
    def _before_task(self, taski, inc_dataset, train_loader=None):
        ...
        if len(self._network.novelty_avgs) != 0:
        # if train_loader is not None or len(self._network.novelty_avgs) != 0:
            ...
            novelty_threshold = self._network.novelty_avgs
            ...
            self._ex.logger.info(f"Center.: {self._network.novelty_avgs}")
            ...
            if is_novel.sum() == novelty_means.shape[-1]:
                ...
                self._head_to_train = novelty_means.shape[-1]
                self.grow_head()
                
                # Set current head to train
                self._ex.logger.info(f"Training new head {self._head_to_train}")
            else:
                self._head_to_train = torch.argmin(novelty_means - novelty_threshold)
                self._ex.logger.info(f"Retraining head {self._head_to_train}")
        else:
            # No tasks trained
        ...
        self._parallel_network.module.active_head = self._head_to_train
    ...
    def get_head_params_with_index(self, idx):
        ...
        for i in range(self._num_heads):
            ...
            if self._parallel_network.module.use_calibration_head and \
                self._parallel_network.module.use_multihead_calibration:
        ...
        # for p in self._parallel_network.module.parameters():
        #     p.requires_grad = True
    ...
    def get_new_optimizers(self):
        ...
        standard_params, calib_params = self.get_head_params_with_index(self._head_to_train)
    ...
    def _train_task(self, train_loader, val_loader):
        ...
        self.freeze_backbone_excluding_index(self._head_to_train)
        ...
        optimizers = self._optimizers[self._head_to_train]
        schedulers = self._schedulers[self._head_to_train]
        warmup_schedulers = self._warmup_schedulers[self._head_to_train]
        ...
        for epoch in range(self._n_epochs):
            ...
            if self._warmup:
                for warmup_scheduler in warmup_schedulers.values():
            ...
            self._network.novelty_avgs.data[self._head_to_train] = 0.0
            self._network.novelty_vars.data[self._head_to_train] = 0.0
            self.num_samples = 0.0
    ...
    def _compute_novelty_loss(self, outputs, inliers, outhead_classes, inhead_classes):
        ...
        if self._parallel_network.module.use_multihead_calibration:
        ...
        if novelty_scores is not None:
            ...
            self._network.novelty_avgs.data[self._head_to_train], \
            self._network.novelty_vars.data[self._head_to_train] = update_exponential_moving_average(
                self._network.novelty_avgs[self._head_to_train], 
                self._network.novelty_vars[self._head_to_train], 
                novelty_scores[:, self._head_to_train]
            )
    ...
    def finetune_last_layer(
        ...
        #     if self._parallel_network.module.use_multihead_calibration:
        #         params.append({
        #             'params': self._parallel_network.module.calibrated_classifiers[self._head_to_train].parameters(),
        #             'lr': self._cfg["novelty_detection"]["optimizer"]["lr"]
        #         })
        #     else:
        #         params.append({
        #             'params': self._parallel_network.module.calibrated_classifiers.parameters(),
        #             'lr': self._cfg["novelty_detection"]["optimizer"]["lr"]
        #         })
