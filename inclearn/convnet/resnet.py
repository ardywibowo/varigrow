...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    return PruningConv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False)


def conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution"""
    return PruningConv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.inplanes = nf
        if 'cifar' in dataset:
            self.conv1 = nn.Sequential(PruningConv2d(3, nf, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(nf), nn.ReLU(inplace=True))
        elif 'imagenet' in dataset:
            if start_class == 0:
                self.conv1 = nn.Sequential(
                    PruningConv2d(3, nf, kernel_size=7, stride=2, padding=3, bias=False),
                    nn.BatchNorm2d(nf),
                ...
                self.conv1 = nn.Sequential(
                    PruningConv2d(3, nf, kernel_size=3, stride=1, padding=1, bias=False),
                    nn.BatchNorm2d(nf),
                    nn.ReLU(inplace=True),
                    nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
                )
