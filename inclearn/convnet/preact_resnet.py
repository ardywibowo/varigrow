...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, remove_last_relu=False):
        ...
        self.conv1 = PruningConv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = PruningConv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes)
        self.remove_last_relu = remove_last_relu

        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                PruningConv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False))
...
class PreActBottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.conv1 = PruningConv2d(in_planes, planes, kernel_size=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = PruningConv2d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes)
        self.conv3 = PruningConv2d(planes, self.expansion * planes, kernel_size=1, bias=False)
...
class PreActResNet(nn.Module):
    def __init__(self,
        ...
        if 'cifar' in dataset:
            self.conv1 = PruningConv2d(3, nf, kernel_size=3, stride=1, padding=1, bias=False)
        else:
            self.conv1 = nn.Sequential(PruningConv2d(3, nf, kernel_size=7, stride=2, padding=3, bias=False),
