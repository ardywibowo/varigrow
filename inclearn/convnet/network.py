...
class BasicNet(nn.Module):
    def __init__(
        ...
        # self.novelty_avgs = Parameter(torch.tensor([]), requires_grad=False).to(self._device)
        # self.novelty_vars = Parameter(torch.tensor([]), requires_grad=False).to(self._device)

        self.novelty_avgs = Parameter(torch.tensor([]), requires_grad=False)
    ...
    def update_novelty_thresholds(self, novelty_avg, novelty_var):
        self.novelty_avgs.data[self._head_to_train] = novelty_avg.clone().detach().to(self.device)
        self.novelty_vars.data[self._head_to_train] = novelty_var.clone().detach().to(self.device)
        
    def grow_novelty_stats(self):
        zero = torch.tensor([0.0]).to(self.novelty_avgs.device)
        self.novelty_avgs.data = torch.cat([self.novelty_avgs.data, zero])
        self.novelty_vars.data = torch.cat([self.novelty_vars.data, zero])